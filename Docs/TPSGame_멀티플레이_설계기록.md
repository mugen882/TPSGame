...
| 항목 | 원인 |
|---|---|
| Cost가 첫 발만 적용 | `CommitAbility`가 활성화 1회에만 호출 |
| 쿨다운이 GAS 밖 | 타이머가 발사 간격을 대신함 |
| 예측 트레이스에 퍼짐 없음 | 권위는 `VRandCone` 난수, 예측은 직선 |

**뿌리가 하나다** — "연사 = 어빌리티 1회 + 타이머" 구조.

M2b-2에서는 응급 대응으로 조준점을 `CachedClientAim`에 캐시하고 서버 `FireLoop`가 발사를 주도하게 했다. 연사 전체가 단일 예측 키를 공유해 TargetData 슬롯이 덮어써지고, 도착 시점에 발사하면 대부분의 발이 유실되기 때문이다. **이 우회 코드는 M2b-3에서 걷어낸다.**

방향: `bRetriggerInstancedAbility = true` + 타이머 제거 + 쿨다운 GE가 발사 간격 담당 + 입력 홀드 중 쿨다운 만료 시 재활성화. 그러면 발당 예측 키가 생겨 Cost·쿨다운·조준점이 전부 라이플과 같은 경로를 탄다.
...
**네트워크 버그는 대부분 조용히 실패한다.** 이 문서에 기록된 12개 함정 중 컴파일 에러나 런타임 경고를 낸 것은 **하나도 없다.** 전부 "그냥 동작하지 않는" 형태였고, 로그 접두사와 마일스톤별 통과 조건이 없었다면 원인 추적에 훨씬 오래 걸렸을 것이다.
