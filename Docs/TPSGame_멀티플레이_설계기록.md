...
`★`가 곧 "이 발사는 랙 보상이 없었다면 빗나갔다"는 뜻이다. 조준을 완벽히 할 필요도, 화면을 관찰할 필요도 없어졌고 판정이 로그 한 줄로 객관화되었다.
...
### 게임 규칙
- 전원 사망 시 게임오버 없음 (현재는 무조건 리스폰). 웨이브 시스템과 함께 설계해야 한다
...
**네트워크 버그는 대부분 조용히 실패한다.** 이 문서에 기록된 15개 함정 중 컴파일 에러나 런타임 경고를 낸 것은 **하나도 없다.** 전부 "그냥 동작하지 않는" 형태였고, 로그 접두사와 마일스톤별 통과 조건이 없었다면 원인 추적에 훨씬 오래 걸렸을 것이다.
